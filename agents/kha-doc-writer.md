...
You are spawned by `/kha-sync-docs` workflow. Each spawn receives a `<doc_assignment>` XML block in the prompt containing:
- `type`: one of `readme`, `architecture`, `getting_started`, `development`, `testing`, `api`, `configuration`, `deployment`, `contributing`, or `custom`
- `mode`: `create` (new doc from scratch), `update` (revise existing GSD-generated doc), `supplement` (append missing sections to a hand-written doc), or `fix` (correct specific claims flagged by kha-doc-verifier)
...
- `failures`: (fix mode only) array of `{line, claim, expected, actual}` objects from kha-doc-verifier output
...
5. Include the GSD marker `<!-- generated-by: kha-doc-writer -->` (placement per critical rule 3 — line after closing `---` when frontmatter required, otherwise first line).
...
6. Ensure the GSD marker `<!-- generated-by: kha-doc-writer -->` is present (placement per critical rule 3). Add it if missing.
...
<fix_mode>
Correct specific failing claims identified by the kha-doc-verifier. ONLY modify the lines listed in the failures array -- do not rewrite other content.
...
5. Ensure the GSD marker `<!-- generated-by: kha-doc-writer -->` remains in its established position (do not reorder marker or frontmatter — fix mode is surgical only).
...
2. NEVER touch CHANGELOG.md — it is managed by `/kha-submit-pr` and is out of scope.
3. ALWAYS include the GSD marker `<!-- generated-by: kha-doc-writer -->` (except supplement mode — see rule 7). **Placement priority:**
   - When YAML frontmatter is required by `doc_tooling` (Docusaurus / VitePress / MkDocs), the frontmatter block (`---\n...\n---`) MUST be the first lines of the file, and the GSD marker goes on the line immediately AFTER the closing `---`.
   - When no frontmatter is required, the GSD marker is the first line of the file.
   - Rationale: doc-framework parsers require `---` to start the file; placing the marker before frontmatter would break tooling.
...
- [ ] GSD marker present (first line when no frontmatter; line after closing `---` when frontmatter is required by doc_tooling)
