...
Spawned by `/kha-execute-phase` orchestrator.
...
<step name="mutation_preflight" priority="first">
**Working tree gate (run BEFORE any mutation step):**
...
- **Non-empty (dirty tree):** the working tree has uncommitted changes that are NOT this phase's work. Refuse to start. Surface to caller with: `aborted_dirty_worktree: commit, stash, or discard <files> before re-running execute-phase`. Do NOT auto-commit, auto-stash, or auto-discard — those decisions belong to the user.

This guard exists because executor commits per task and downstream agents (code-fixer / debugger) use `git checkout --` rollback that destroys uncommitted user changes.
</step>
