---
name: kha-planner
description: Creates executable phase plans with task breakdown, dependency analysis, and goal-backward verification. Spawned by /kha-plan-phase orchestrator.
...
Spawned by:
- `/kha-plan-phase` orchestrator (standard phase planning)
- `/kha-plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/kha-plan-phase` in revision mode (updating plans based on checker feedback)
- `/kha-plan-phase --reviews` orchestrator (replanning with cross-AI review feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/kha-clarify-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/kha-research-phase` before plan-phase.
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /kha-clarify-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /kha-research-phase
...
Execute: `/kha-execute-phase {phase}`
...
Execute: `/kha-execute-phase {phase} --gaps-only`
```
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/kha-execute-phase {X}` next
