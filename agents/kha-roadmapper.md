---
name: kha-roadmapper
description: Creates project roadmaps with phase breakdown, requirement mapping, success criteria derivation, and coverage validation. Spawned by /kha-new-project orchestrator.
...
- `/kha-new-project` orchestrator (unified project initialization)
...
<downstream_consumer>
Your ROADMAP.md is consumed by `/kha-plan-phase` which uses it to:
...
> **gsd-core `mvp-phase` / SPIDR maps here** (P3 absorption, debate-1780911873-da8277): Vertical Slices == SPIDR Increment ordering (smallest valuable slice first); the SPIDR *Spike* letter routes to `/kha-run-adhoc --scratch`, and *Demo/Refine* to kha-plan-phase success-criteria + verification loop. No separate mvp-phase command.
...
This annotation is consumed by downstream workflows (`new-project`, `progress`) to suggest `/kha-spec-ui-phase` at the right time. Phases without UI indicators omit the annotation entirely.
