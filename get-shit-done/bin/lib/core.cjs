 ...
 *
 * Recognises both standard format (kha-planner.md) and Copilot format
 * (kha-planner.agent.md). Copilot renames agent files during install (#1512).
 *
...
function checkAgentsInstalled() {
  ...
  // MODEL_PROFILES keys are gsd-* agent TYPES (used for model resolution); the
  // actual agent FILES on disk are kha-* (the gsd→kha rename was a pure prefix
  // swap for agents). Map type → file base so the on-disk check matches reality.
  const expectedAgents = Object.keys(MODEL_PROFILES).map((a) => a.replace(/^gsd-/, 'kha-'));
  const installed = [];
  const missing = [];
