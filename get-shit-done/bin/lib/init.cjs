...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'kha-executor'),
    verifier_model: resolveModelInternal(cwd, 'kha-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'kha-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'kha-planner'),
    checker_model: resolveModelInternal(cwd, 'kha-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'kha-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'kha-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'kha-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'kha-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /kha-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /kha-new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/kha-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/kha-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/kha-clarify-phase ${phase.number}`,
      });
    }
  }
 ...
 * @param {string} agentType - The agent type (e.g., 'kha-executor', 'kha-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills kha-executor 2>/dev/null)
 */
