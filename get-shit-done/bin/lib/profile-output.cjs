...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /kha-new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  ...
  '- `/kha-run-adhoc` for small fixes, doc updates, and ad-hoc tasks',
  '- `/kha-debug` for investigation and bug fixing',
  '- `/kha-execute-phase` for planned phase work',
  '',
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/kha-user-profile` to generate your developer profile.',
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/kha-user-profile --refresh` with session data to populate.';
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GSD from ${dataSource}. Run \`/kha-user-profile --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /kha-user-profile to unlock Developer Profile.';
