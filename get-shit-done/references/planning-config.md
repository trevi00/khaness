...
| `manager.flags.discuss` | `""` | Flags passed to `/kha-clarify-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/kha-clarify-phase` from manager (e.g., `"--auto --analyze"`) |
