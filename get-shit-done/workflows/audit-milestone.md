...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-integration-checker --raw)
```
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="kha-integration-checker",
...
Discovery only — never auto-calls `/kha-validate-nyquist-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/kha-validate-nyquist-phase {N}` |

Phases needing validation: run `/kha-validate-nyquist-phase {N}` for each flagged phase.
...
- /kha-complete-milestone {version} — proceed anyway (accept tech debt)
