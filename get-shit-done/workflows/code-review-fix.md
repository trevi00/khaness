<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns kha-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- kha-code-fixer: Applies fixes to code review findings
- kha-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /kha-review-code ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the kha-code-fixer agent with config:
...
```
Task(subagent_type="kha-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /kha-remediate-code-review ${PHASE_ARG}.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn kha-code-reviewer agent to re-review
    # (This overwrites REVIEW_PATH with latest review state)
    Task(subagent_type="kha-code-reviewer", prompt="
<config>
depth: ${REVIEW_DEPTH}
phase_dir: ${PHASE_DIR}
review_path: ${REVIEW_PATH}
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /kha-remediate-code-review ${PHASE_ARG}"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /kha-verify-uat  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /kha-review-code ${PHASE_NUMBER}           — Re-review code"
  echo "  /kha-verify-uat                           — Verify phase completion"
  echo ""
fi
```
