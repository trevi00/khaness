<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns kha-code-reviewer agent, commits REVIEW.md, and presents results to user.
</purpose>
...
- kha-code-reviewer: Reviews source files for bugs and quality issues
</available_agent_types>
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /kha-review-code ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
Spawn the kha-code-reviewer agent:
...
No REVIEW.md created. You can retry with /kha-review-code ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /kha-review-code ${PHASE_ARG}"
fi
```
</step>
...
Next steps:
  /kha-remediate-code-review ${PHASE_NUMBER}  — Auto-fix issues
