...
- kha-assumptions-analyzer — Analyzes codebase to surface implementation assumptions
</available_agent_types>
...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-assumptions-analyzer 2>/dev/null)
```
...
Use /kha-status to see available phases.
```
Exit workflow.
...
- When spawning the kha-assumptions-analyzer, pass the lens list so it can flag which lenses apply
...
Spawn a `kha-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="kha-assumptions-analyzer", prompt="""
...
`/kha-plan-phase ${PHASE}`

---

**Also available:**
- `/kha-plan-phase ${PHASE} --skip-research` — plan without research
- `/kha-spec-ui-phase ${PHASE}` — generate UI design contract (if frontend work)
...
Launch: `Skill(skill="kha-plan-phase", args="${PHASE} --auto")`
