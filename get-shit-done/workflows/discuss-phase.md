...
1. **kha-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **kha-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/kha-clarify-phase --text`)
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/kha-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-advisor-researcher 2>/dev/null)
```
...
Use /kha-status ${GSD_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /kha-plan-phase {X} ${GSD_WS} to replan
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @$HOME/.claude/agents/kha-advisor-researcher.md for your role and instructions.
...
This is a structured checkpoint, not the final CONTEXT.md — the `write_context` step still produces the canonical output. But if the session dies, the next `/kha-clarify-phase` invocation can detect this checkpoint and offer to resume from it instead of starting from scratch.
...
`/kha-plan-phase ${PHASE} ${GSD_WS}`

---

**Also available:**
- `/kha-clarify-phase ${PHASE} --chain ${GSD_WS}` — re-run with auto plan+execute after
- `/kha-plan-phase ${PHASE} --skip-research ${GSD_WS}` — plan without research
- `/kha-spec-ui-phase ${PHASE} ${GSD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```
Skill(skill="kha-plan-phase", args="${PHASE} --auto ${GSD_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /kha-clarify-phase ${NEXT_PHASE} ${WAS_CHAIN ? "--chain" : "--auto"} ${GSD_WS}
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /kha-execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /kha-plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /kha-plan-phase ${PHASE} --gaps ${GSD_WS}
  ```
