...
- **Claude Code:** Uses `Task(subagent_type="kha-executor", ...)` — blocks until complete, returns result
...
- kha-debugger — Diagnoses and fixes issues
- kha-codebase-mapper — Maps project structure and dependencies
- kha-integration-checker — Checks cross-phase integration
- kha-nyquist-auditor — Validates verification coverage
- kha-ui-researcher — Researches UI/UX approaches
- kha-ui-checker — Reviews UI implementation quality
- kha-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
AGENT_SKILLS=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-executor 2>/dev/null)
```
...
Check if the current runtime is Copilot by testing for the `@kha-executor` agent pattern
...
⚠ Security enforcement enabled — run before advancing:
  /kha-verify-security-phase {PHASE} ${GSD_WS}
```

If `SECURITY_CFG` is `true` AND SECURITY.md exists: check frontmatter `threats_open`. If > 0:
```
⚠ Security gate: {threats_open} threats open
  /kha-verify-security-phase {PHASE} — resolve before advancing
```
</step>
...
/kha-execute-phase {phase} ${GSD_WS}                # Continue remaining waves
/kha-execute-phase {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
**Invoke review:**
```
Skill(skill="kha-review-code", args="${PHASE_NUMBER}")
```
...
/kha-remediate-code-review ${PHASE_NUMBER}
```
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-verifier 2>/dev/null)
```
...
| `gaps_found` | Present gap summary, offer `/kha-plan-phase {phase} --gaps ${GSD_WS}` |
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/kha-status` and `/kha-audit-uat-backlog`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/kha-verify-uat` on it.
...
`/kha-plan-phase {X} --gaps ${GSD_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/kha-verify-uat {X} ${GSD_WS}` — manual testing first
```

Gap closure cycle: `/kha-plan-phase {X} --gaps ${GSD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/kha-execute-phase {X} --gaps-only ${GSD_WS}` → verifier re-runs.
</step>
...
These items are tracked and will appear in `/kha-status` and `/kha-audit-uat-backlog`.
```
...
> **kha harness note (2026-06-10):** `features.global_learnings` is **OFF by default and is
> currently a no-op in this harness** — no workflow produces a `LEARNINGS.md` file (the
> producer, gsd-core's `extract-learnings`, was deliberately NOT absorbed; see
> `example_project-analysis/kha-gsd-gap-CLOSURE-2026-06-09.md` §cross-cutting #1). The consumer
> below stays wired so the feature can be revived, but enabling the flag has no effect until a
> `LEARNINGS.md` producer is added. **Do not enable expecting results.**
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/kha-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
/kha-status ${GSD_WS} — see updated roadmap
/kha-clarify-phase {next} ${GSD_WS} — discuss next phase before planning
/kha-plan-phase {next} ${GSD_WS} — plan next phase
/kha-execute-phase {next} ${GSD_WS} — execute next phase
```
...
<resumption>
Re-run `/kha-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
