...
2. Per segment:
   - Subagent route: spawn kha-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/kha-execute-phase {phase}` + `/kha-verify-uat`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/kha-plan-phase {Z+1}` + `/kha-verify-uat {Z}` + `/kha-clarify-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/kha-complete-milestone` + `/kha-verify-uat` + `/kha-add-phase` |
