...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via kha-plan-checker
...
```
Usage: /kha-import-plan --from <path>
...
Delegate validation to kha-plan-checker:
...
- Use `pbr:plan-checker` or `pbr:planner` — use `kha-plan-checker` and `kha-planner`
