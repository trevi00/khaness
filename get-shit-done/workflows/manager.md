...
- `manager_flags.discuss` — appended to `/kha-clarify-phase` args (e.g. `"--auto --analyze"`)
...
All {phase_count} phases done. Ready for final steps:
  → /kha-verify-uat — run acceptance testing
  → /kha-complete-milestone — archive and wrap up
```
...
Handle responses:
- "Verify work": `Skill(skill="kha-verify-uat")`  then loop to dashboard.
- "Complete milestone": `Skill(skill="kha-complete-milestone")` then exit.
...
```
Skill(skill="kha-clarify-phase", args="{PHASE_NUM} {manager_flags.discuss}")
```
...
Skill(skill=\"kha-plan-phase\", args=\"{N} --auto {manager_flags.plan}\")
...
Skill(skill=\"kha-execute-phase\", args=\"{N} {manager_flags.execute}\")
...
- Display the error clearly, then offer to fix it:
  ...
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `Skill(skill="kha-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="kha-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `Skill(skill="kha-plan-phase", args="{N}")` if the failed action was planning, or `Skill(skill="kha-execute-phase", args="{N}")` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume anytime: /kha-milestone-manager
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Note:** Any background agents still running will continue to completion. Their results will be visible on next `/kha-milestone-manager` or `/kha-status` invocation.
