...
AGENT_SKILLS_MAPPER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-codebase-mapper 2>/dev/null)
```
...
Spawn 4 parallel kha-codebase-mapper agents.

Use Task tool with `subagent_type="kha-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `kha-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.

**Agent 1: Tech Focus**

```
Task(
  subagent_type="kha-codebase-mapper",
  model="{mapper_model}",
...
Use the same document templates as the `kha-codebase-mapper` agent. Include actual file paths formatted with backticks.
...
**Also available:**
- Re-run mapping: `/kha-map-codebase`
...
- If Task tool available: 4 parallel kha-codebase-mapper agents spawned with run_in_background=true
