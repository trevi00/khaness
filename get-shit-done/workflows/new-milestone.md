...
- Tell the user to complete/archive the previous milestone first, then rerun `/kha-new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/kha-plan-phase` behavior. To change the default, use `/kha-settings`.
...
Spawn 4 parallel kha-project-researcher agents. Each uses this template with dimension-specific fields:
...
", subagent_type="kha-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
`/kha-clarify-phase [N] ${GSD_WS}` — gather context and clarify approach

Also: `/kha-plan-phase [N] ${GSD_WS}` — skip discussion, plan directly
```
...
- [ ] kha-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/kha-clarify-phase [N] ${GSD_WS}`
