...
- kha-project-researcher — Researches project-level technical decisions
- kha-research-synthesizer — Synthesizes findings from parallel research agents
- kha-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
- File reference: `/kha-new-project --auto @prd.md`
...
Usage:
  /kha-new-project --auto @your-idea.md
  /kha-new-project --auto [paste or write your idea here]
...
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-research-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-roadmapper 2>/dev/null)
```
...
**If `project_exists` is true:** Error — project already initialized. Use `/kha-status`.
...
- options:
  - "Map codebase first" — Run /kha-map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/kha-map-codebase` first, then return to `/kha-new-project`
```
...
**After each milestone** (via `/kha-complete-milestone`):
...
**Note:** Run `/kha-settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
Spawn 4 parallel kha-project-researcher agents with path references:
...
", subagent_type="kha-project-researcher", model="{researcher_model}", description="Stack research")
...
", subagent_type="kha-project-researcher", model="{researcher_model}", description="Features research")
...
", subagent_type="kha-project-researcher", model="{researcher_model}", description="Architecture research")
...
", subagent_type="kha-project-researcher", model="{researcher_model}", description="Pitfalls research")
```
...
", subagent_type="kha-research-synthesizer", model="{synthesizer_model}", description="Synthesize research")
```
...
Spawn kha-roadmapper agent with path references:
...
", subagent_type="kha-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="kha-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
Exit skill and invoke SlashCommand("/kha-clarify-phase 1 --auto")
...
/kha-clarify-phase 1 — gather context and clarify approach

---

**Also available:**
- /kha-spec-ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /kha-plan-phase 1 — skip discussion, plan directly
...
- [ ] kha-roadmapper spawned with context
...
- [ ] User knows next step is `/kha-clarify-phase 1`
