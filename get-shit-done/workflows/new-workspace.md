...
Run this command from a directory containing git repos, or specify repos explicitly:
  /kha-new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /kha-new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
Next steps:
  cd $TARGET_PATH
  /kha-new-project    # Initialize GSD in the workspace
```
...
- options:
  - "Yes — run /kha-new-project" → tell user to `cd $TARGET_PATH` first, then run `/kha-new-project`
