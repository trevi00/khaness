...
```
No GSD project detected. Run `/kha-new-project` to get started.
```
Exit.
</step>
...
Run `/kha-audit-planning-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
- If file exists and count >= 6: prompt "You've called /kha-advance {N} times consecutively. Continue? [y/N]"
- If user says no, exit
- Increment the counter
- The counter file is deleted by any non-`/kha-advance` command (convention — other workflows don't need to implement this, the note here is sufficient)
</step>
...
→ Next action: `/kha-clarify-phase <first-phase>`
...
Do not ask for confirmation — the whole point of `/kha-advance` is zero-friction advancement.
</step>
