<purpose>
Create all phases necessary to close gaps identified by `/kha-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/kha-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/kha-audit-milestone` first.
```
...
**Also available:**
- `/kha-execute-phase {N}` — if plans already exist
...
`/kha-audit-milestone` — re-audit to verify gaps closed
`/kha-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/kha-plan-phase` next
</success_criteria>
