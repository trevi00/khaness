<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates kha-phase-researcher, kha-planner, and kha-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-phase-researcher 2>/dev/null)
...
**If `planning_exists` is false:** Error — run `/kha-new-project` first.
...
/kha-review-plan-peer --phase {N}

Then re-run /kha-plan-phase {N} --reviews
```
Exit workflow.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /kha-plan-phase {X} ${GSD_WS}:

  /kha-clarify-phase {X} ${GSD_WS}
  ```
...
<files_to_read>
- {context_path} (USER DECISIONS from /kha-clarify-phase)
...
```
Skill(skill="kha-spec-ui-phase", args="${PHASE} --auto ${GSD_WS}")
```
...
1. Generate UI-SPEC first — Run /kha-spec-ui-phase {N} then re-run /kha-plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/kha-spec-ui-phase {N} ${GSD_WS}` then re-run `/kha-plan-phase {N} ${GSD_WS}`". Exit workflow.
...
1. Re-run: `/kha-plan-phase {PHASE} --research ${GSD_WS}`
...
<downstream_consumer>
Output consumed by /kha-execute-phase. Plans need:
...
**If "Split":** Use `/kha-insert-phase` to create the sub-phases, then replan each.
...
## 10. Spawn kha-plan-checker Agent
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/kha-debug` to investigate root causes."
...
```
Skill(skill="kha-execute-phase", args="${PHASE} --auto --no-transition ${GSD_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /kha-clarify-phase ${NEXT_PHASE} --auto ${GSD_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /kha-execute-phase ${PHASE} ${GSD_WS}
  ```
...
/kha-execute-phase {X} ${GSD_WS}

───────────────────────────────────────────────────────────────

**Also available:**
- cat .planning/phases/{phase-dir}/*-PLAN.md — review plans
- /kha-plan-phase {X} --research — re-research first
- /kha-review-plan-peer --phase {X} --all — peer review plans with external AIs
- /kha-plan-phase {X} --reviews — replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/kha-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/kha-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] kha-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] kha-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] kha-plan-checker spawned with CONTEXT.md
