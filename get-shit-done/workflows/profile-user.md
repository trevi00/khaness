...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing gsd-tools.cjs subcommands and the kha-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
- @$HOME/.claude/get-shit-done/agents/kha-user-profiler.md (profiler agent definition)
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /kha-user-profile when ready." and exit
...
**Spawn kha-user-profiler agent using Task tool:**

Use the Task tool to spawn the `kha-user-profiler` agent. Provide it with:
