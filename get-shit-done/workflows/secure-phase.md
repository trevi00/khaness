...
- kha-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-security-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-security-auditor --raw)
...
If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /kha-settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /kha-execute-phase {N} first."
...
## 5. Spawn kha-security-auditor

```
Task(
  prompt="Read $HOME/.claude/agents/kha-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="kha-security-auditor",
  model="{AUDITOR_MODEL}",
...
▶ Fix mitigations then re-run: /kha-verify-security-phase {N}
...
▶ /kha-validate-nyquist-phase {N}    validate test coverage
▶ /kha-verify-uat {N}       run UAT
```
