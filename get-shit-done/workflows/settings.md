...
- `workflow.ui_safety_gate` — prompt to run /kha-spec-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /kha-spec-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /kha-spec-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /kha-run-milestone — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
...
These settings apply to future /kha-plan-phase and /kha-execute-phase runs.

Quick commands:
- /kha-set-model-profile <profile> — switch model profile
- /kha-plan-phase --research — force research
- /kha-plan-phase --skip-research — skip research
- /kha-plan-phase --skip-verify — skip plan check
```
</step>
