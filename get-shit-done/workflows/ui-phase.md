<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates kha-ui-researcher and kha-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-ui-checker --raw)
```
...
```
UI phase is disabled in config. Enable via /kha-settings.
```
Exit workflow.
...
Recommended: run /kha-clarify-phase {N} first to capture design preferences.
...
```markdown
Read $HOME/.claude/agents/kha-ui-researcher.md for instructions.
...
Task(
  ...
  subagent_type="kha-ui-researcher",
...
```markdown
Read $HOME/.claude/agents/kha-ui-checker.md for instructions.
...
- Re-spawn kha-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /kha-spec-ui-phase
...
- [ ] kha-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] kha-ui-checker spawned with UI-SPEC.md
