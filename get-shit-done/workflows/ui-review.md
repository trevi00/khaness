...
AGENT_SKILLS_UI_REVIEWER=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills kha-ui-auditor 2>/dev/null)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`, `commit_docs`.

```bash
UI_AUDITOR_MODEL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model kha-ui-auditor --raw)
```
...
**If `SUMMARY_FILES` empty:** Exit — "Phase {N} not executed. Run /kha-execute-phase {N} first."
...
```markdown
Read $HOME/.claude/agents/kha-ui-auditor.md for instructions.
...
- `/kha-verify-uat {N}` — UAT testing
- `/kha-plan-phase {N+1}` — plan next phase
...
- [ ] kha-ui-auditor spawned with correct context
