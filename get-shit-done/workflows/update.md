...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/kha-self-update` targets the runtime that invoked it.
...
If you've modified any GSD files directly, they'll be automatically backed up to `gsd-local-patches/` and can be reapplied with `/kha-reapply-local-patches` after the update.
```
...
Run /kha-reapply-local-patches to merge your modifications into the new version.
```
