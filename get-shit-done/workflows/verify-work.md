<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /kha-plan-phase --gaps.
...
Provide a phase number to start testing (e.g., /kha-verify-uat 4)
```
...
```
⚠ Security enforcement enabled — /kha-verify-security-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/kha-verify-security-phase {phase}` — security review (required before advancing)
- `/kha-plan-phase {next}` — Plan next phase
- `/kha-execute-phase {next}` — Execute next phase
- `/kha-review-ui {phase}` — visual quality audit (if frontend files were modified)
```
...
- `/kha-plan-phase {next}` — Plan next phase
- `/kha-execute-phase {next}` — Execute next phase
- `/kha-verify-security-phase {phase}` — security review
- `/kha-review-ui {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
Spawn kha-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /kha-execute-phase
...
Spawn kha-planner with revision context:
...
3. Abandon (exit, user runs /kha-plan-phase manually)
...
`/clear` then `/kha-execute-phase {phase} --gaps-only`
...
- [ ] If issues: kha-planner creates fix plans (gap_closure mode)
- [ ] If issues: kha-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/kha-execute-phase --gaps-only` when complete
</success_criteria>
