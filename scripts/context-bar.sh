...
# Based on: https://github.com/ykdojo/claude-code-tips/blob/main/scripts/context-bar.sh
COLOR="blue"
...
# Windows Git Bash: ensure jq is on PATH (winget installs to non-standard location)
# Use USERPROFILE instead of HOME (HOME may point to HOME_REDIRECT on some Windows setups)
_REAL_HOME="${USERPROFILE:-${HOMEDRIVE}${HOMEPATH}}"
_REAL_HOME="${_REAL_HOME//\\//}"  # convert backslashes to forward slashes
JQ_WINGET="${_REAL_HOME}/AppData/Local/Microsoft/WinGet/Packages/jqlang.jq_Microsoft.Winget.Source_8wekyb3d8bbwe/jq.exe"
if ! command -v jq &>/dev/null && [[ -x "$JQ_WINGET" ]]; then
    jq() { "$JQ_WINGET" "$@"; }
fi
    ...
    if [[ -n "$branch" ]]; then
        ...
        if [[ -n "$upstream" ]]; then
            ...
            if [[ -f "$fetch_head" ]]; then
                # Windows Git Bash: stat -c %Y works, macOS uses stat -f %m
