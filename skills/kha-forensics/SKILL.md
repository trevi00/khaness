...
- read-only forensic gap report and priority-ordered anomaly list grounded in commits, artifacts, and state evidence; primary output is inline markdown, with optional persisted `.planning/forensics/report-<ts>.md` only when the user asks to save evidence.

## Failure behavior


- missing git history or `.planning/` artifacts reduces evidence scope and is called out explicitly; insufficient evidence returns `inconclusive`; redaction problems suppress the sensitive excerpt instead of printing it.

## Gate summary


- inspect git history, artifact completeness, session reports, and worktree state only; no source edits, no issue creation, and no STATE mutation in audit mode.

## Retry / Resume


- rerun after narrowing the problem statement or after additional evidence exists; idempotent except for optional saved-report timestamps.
