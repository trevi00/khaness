...
Then run /kha-intel-index refresh to build the initial index.
```
...
- If no matches found, display: `No intel matches for '<term>'. Try /kha-intel-index refresh to build the index.`
...
Task(
  ...
  prompt="You are the kha-intel-updater agent. Your job is to analyze this codebase and write/update intelligence files in .planning/intel/.
...
- artifacts: inline results for `query`, `status`, or `diff`; on `refresh`, `.planning/intel/` store refreshed using the implemented intel file set (`files.json`, `apis.json`, `deps.json`, `stack.json`, and current `arch.md` slot) plus post-refresh freshness summary.
- status: `intel_disabled` | `intel_queried` | `intel_status_emitted` | `intel_diff_emitted` | `intel_refreshed`.

## Failure behavior


- preflight: if `.planning/config.json` is missing or `intel.enabled` is not explicitly `true`, abort mutation and show the enablement command.
- execution: if refresh agent work fails, preserve existing intel files and report which artifacts are stale or missing.
- partial: query/status/diff are read-only and should never mutate; refresh may leave a mixed-generation set, which must be surfaced explicitly instead of masked as success.

## Gate summary


- preflight: config gate passed; mode resolved from `$ARGUMENTS`; `query/status/diff` stay inline and `refresh` is the only mutating path.
- success: dry-run for `refresh` includes current freshness or diff view; snapshot is the pre-refresh intel state from `status`/`diff`; explicit refresh confirmation is required before rebuilding `.planning/intel/`.
- boundary: own incremental machine-oriented intel under `.planning/intel/`; `kha-scan-codebase` is a short focused narrative scan; `kha-map-codebase` is the deep full 7-document narrative map.
