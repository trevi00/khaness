---
name: kha-reapply-local-patches
...
Local patches are automatically saved when you run /kha-self-update
...
- artifact: merged live files at their installed destinations, driven by `gsd-local-patches/backup-meta.json` and the backed-up file copies; valid per-file end states are `Merged`, `Conflict resolved`, or `Incorporated`, never `Skipped/no custom content`.
- artifact: before overwriting any file, a pre-merge snapshot of the current installed version must be surfaced for that file along with the chosen merge strategy (`three-way (git)`, `three-way (pristine)`, or `two-way (enhanced)`).
- artifact: an end-of-run per-file result table, with optional cleanup of `gsd-local-patches/` only if the user chooses it.
- status: `no_patches` | `reapply_complete` | `reapply_partial` | `reapply_conflicts`

## Failure behavior


- preflight failure: if no patches dir or no readable `backup-meta.json` is found, exit cleanly with no writes.
- execution failure: if a file cannot be merged confidently, classify it as `Conflict` and stop or defer that file without silently skipping it; keep the backup copy intact and preserve any files already merged earlier in the run.
- partial success: merged files stay active, unresolved files remain backed up and resumable file-by-file, and any hunk-drop warnings must be surfaced rather than hidden.

## Gate summary


- preflight: `gsd-local-patches/` is detected, `backup-meta.json` is readable, and a baseline strategy is chosen before merge work begins.
- success criteria: every backed-up file is processed, each file gets a pre-merge snapshot before overwrite, user customizations are preserved or explicitly conflict-resolved, post-merge verification runs, and no file is ever labeled “no custom content.”

## Retry / Resume


- checkpoint: `backup-meta.json`, the backed-up file copies under `gsd-local-patches/`, and the per-file status from the last run.
- resume command: rerun `/kha-reapply-local-patches`; already merged/incorporated files should be treated as done, and the run should continue only with unresolved or conflict files.
- idempotent: per-file resume is conditionally yes while the target file still matches the snapshot used for that file; whole-run no after extra manual edits or another update.

### dry-run mode
- `--dry-run` lists each local patch by file path + diff hunk count. Shows
  which files will be merged, which will conflict, which will skip.
- `--apply` does the merge.

### per-file snapshot
- Before merging each file, write the original to
  `.planning/snapshots/reapply-<ts>/<rel-path>.orig`. Allows per-file revert
  without losing earlier successful merges.

### per-file checkpoint
- `.planning/checkpoints/reapply-local-patches.jsonl` — one line per file:
  `{path, status: ok|conflict|skipped, ts}`.
- Resume scans the checkpoint and skips files marked `ok` or `skipped`.

### conflict handling
- On conflict, halt at that file, write per-file conflict marker, surface
  manual resolution required. Already-merged files stay merged.
